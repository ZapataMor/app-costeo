    ...
    return (
        <>
            ...
            <div className="flex flex-col gap-4 p-4">
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b bg-muted/50 text-left text-muted-foreground">
                                ...
                                <th className="p-3 text-right font-medium">Duración est.</th>
                                <th className="p-3 text-right font-medium">Tarifa SOAT</th>
                        ...
                        <tbody>
                            ...
                            {procedimientos.data.map((proc) => (
                                <tr key={proc.id} className="border-b last:border-0">
                                    <td className="p-3 font-mono text-xs">{proc.codigo_cups}</td>
                                    <td className="p-3">{proc.nombre}</td>
                                    <td className="p-3">{proc.especialidad}</td>
                                    <td className="p-3 capitalize">{proc.complejidad}</td>
                                    <td className="p-3 text-right tabular-nums">{proc.duracion_estimada_minutos} min</td>
                                    <td className="p-3 text-right tabular-nums">
                                        {proc.tarifa_soat ? cop(Number(proc.tarifa_soat)) : '—'}
                                    </td>
                                    <td className="p-3">
                                        <NivelConfiabilidadBadge nivel={proc.nivel_confiabilidad} />
                                    </td>
                                    <td className="max-w-48 truncate p-3 text-muted-foreground" title={proc.fuente ?? ''}>
                                        {proc.fuente ?? '—'}
                                    </td>
                                    <td className="p-3 text-right whitespace-nowrap">
                                        <Button asChild variant="ghost" size="icon" aria-label="Editar">
                                            <Link href={ProcedimientoQuirurgicoController.edit.url(proc.id)} prefetch>
                                                <Pencil className="size-4" />
                                            </Link>
                                        </Button>
                                        <ConfirmarEliminacion
                                            url={ProcedimientoQuirurgicoController.destroy.url(proc.id)}
                ...
                <Paginacion links={procedimientos.links} total={procedimientos.total} from={procedimientos.from} to={procedimientos.to} />
            </div>
        </>
    );
}
...
ProcedimientosIndex.layout = {
    breadcrumbs: [{ title: 'Procedimientos', href: '/parametros/procedimientos' }],
};
