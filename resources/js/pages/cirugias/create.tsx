...
type MiembroFila = { recurso_humano_id: string; rol: string; minutos_participacion: string };
...
export default function CirugiasCreate({
    ...
    const error = (clave: string): string | undefined => (errors as Record<string, string>)[clave];
    ...
    const actualizarFila = <K extends 'procedimientos' | 'equipo' | 'consumos' | 'equipos_medicos'>(
        campo: K,
        indice: number,
        cambios: Partial<DatosCirugia[K][number]>,
    ) => {
        const filas = data[campo].map((fila, i) => (i === indice ? { ...fila, ...cambios } : fila));
        setData(campo, filas as never);
    };

    const quitarFila = (campo: 'procedimientos' | 'equipo' | 'consumos' | 'equipos_medicos', indice: number) => {
                ...
                <form onSubmit={enviar} className="max-w-4xl space-y-4">
                    <Card>
                        ...
                        <CardContent className="grid gap-4 sm:grid-cols-2">
                            <div className="grid gap-2">
                                ...
                                <Select value={data.paciente_id} onValueChange={(v) => setData('paciente_id', v)}>
                                    <SelectTrigger>
                            ...
                            <div className="grid gap-2">
                                ...
                                <Select value={data.sala_operatoria_id} onValueChange={(v) => setData('sala_operatoria_id', v)}>
                                    <SelectTrigger>
                            ...
                            <div className="grid gap-2">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Select value={data.estado} onValueChange={(v) => setData('estado', v)}>
                                        ...
                                        <SelectContent>
                                            {estados.map((est) => (
                                                <SelectItem key={est} value={est} className="capitalize">
                                                    {est}
                                                </SelectItem>
                                            ))}
                                        </SelectContent>
                                ...
                                <InputError message={error('tipo') ?? error('estado')} />
                            </div>
                            <div className="grid gap-2">
                            ...
                            <div className="grid gap-2">
                                <Label htmlFor="diagnostico_cie10">Diagnóstico CIE-10 (opcional)</Label>
                                <Input
                                    id="diagnostico_cie10"
                                    value={data.diagnostico_cie10}
                                    onChange={(e) => setData('diagnostico_cie10', e.target.value)}
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Al menos uno; marca cuál es el principal.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            ...
                            {data.procedimientos.map((fila, i) => (
                                <div key={i} className="flex flex-wrap items-center gap-2">
                                    <div className="min-w-64 flex-1">
                                        <Select value={fila.id} onValueChange={(v) => actualizarFila('procedimientos', i, { id: v })}>
                                            <SelectTrigger>
                                            ...
                                            <SelectContent>
                                                {procedimientos.map((p) => (
                                                    <SelectItem key={p.id} value={String(p.id)}>
                                                        {p.codigo_cups} · {p.nombre} ({p.duracion_estimada_minutos} min est.)
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                        <InputError message={error(`procedimientos.${i}.id`)} />
                                    </div>
                                    <label className="flex items-center gap-1.5 text-sm">
                                        <Checkbox
                                            ...
                                            onCheckedChange={(v) =>
                                                setData(
                                                    'procedimientos',
                                                    data.procedimientos.map((p, j) => ({ ...p, es_principal: j === i && v === true })),
                                                )
                                            }
                                        />
                                        Principal
                                    </label>
                                    <Button type="button" variant="ghost" size="icon" aria-label="Quitar" onClick={() => quitarFila('procedimientos', i)}>
                            ...
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={() => setData('procedimientos', [...data.procedimientos, { id: '', es_principal: false }])}
                            >
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Personal que participó y sus minutos (base del costo TDABC de talento humano).</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3">
                                ...
                                <div key={i} className="flex flex-wrap items-center gap-2">
                                    <div className="min-w-56 flex-1">
                                        <Select
                                            ...
                                            onValueChange={(v) => {
                                                const recurso = recursos.find((r) => String(r.id) === v);
                                        ...
                                        <InputError message={error(`equipo.${i}.recurso_humano_id`)} />
                                    </div>
                                    <div className="w-44">
                                        <Select value={fila.rol} onValueChange={(v) => actualizarFila('equipo', i, { rol: v })}>
                                            <SelectTrigger>
                                    ...
                                    <div className="w-32">
                                        <Input
                                            ...
                                            onChange={(e) => actualizarFila('equipo', i, { minutos_participacion: e.target.value })}
                                        />
                                        <InputError message={error(`equipo.${i}.minutos_participacion`)} />
                                    </div>
                                    <Button type="button" variant="ghost" size="icon" aria-label="Quitar" onClick={() => quitarFila('equipo', i)}>
                                        <Trash2 className="size-4 text-destructive" />
                                    </Button>
                                </div>
                            ))}
                            <Button
                                ...
                                size="sm"
                                onClick={() =>
                                    setData('equipo', [...data.equipo, { recurso_humano_id: '', rol: '', minutos_participacion: '' }])
                                }
                            >
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>El precio se toma del insumo al momento de registrar (snapshot).</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            {data.consumos.map((fila, i) => (
                                <div key={i} className="flex flex-wrap items-center gap-2">
                                    <div className="min-w-64 flex-1">
                                        <Select value={fila.insumo_id} onValueChange={(v) => actualizarFila('consumos', i, { insumo_id: v })}>
                                            <SelectTrigger>
                                            ...
                                            <SelectContent>
                                                {insumos.map((ins) => (
                                                    <SelectItem key={ins.id} value={String(ins.id)}>
                                                        {ins.codigo} · {ins.nombre} ({ins.unidad})
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                        <InputError message={error(`consumos.${i}.insumo_id`)} />
                                    </div>
                                    <div className="w-32">
                                        <Input
                                            type="number"
                                            ...
                                            value={fila.cantidad}
                                            onChange={(e) => actualizarFila('consumos', i, { cantidad: e.target.value })}
                                        />
                                        <InputError message={error(`consumos.${i}.cantidad`)} />
                                    </div>
                                    <Button type="button" variant="ghost" size="icon" aria-label="Quitar" onClick={() => quitarFila('consumos', i)}>
                                        <Trash2 className="size-4 text-destructive" />
                                    </Button>
                                </div>
                            ))}
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={() => setData('consumos', [...data.consumos, { insumo_id: '', cantidad: '' }])}
                            >
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Equipos usados y sus minutos de uso.</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-3">
                            {data.equipos_medicos.map((fila, i) => (
                                <div key={i} className="flex flex-wrap items-center gap-2">
                                    <div className="min-w-64 flex-1">
                                        <Select value={fila.id} onValueChange={(v) => actualizarFila('equipos_medicos', i, { id: v })}>
                                            <SelectTrigger>
                                        ...
                                        <InputError message={error(`equipos_medicos.${i}.id`)} />
                                    </div>
                                    <div className="w-32">
                                        <Input
                                            type="number"
                                            ...
                                            onChange={(e) => actualizarFila('equipos_medicos', i, { minutos_uso: e.target.value })}
                                        />
                                        <InputError message={error(`equipos_medicos.${i}.minutos_uso`)} />
                                    </div>
                                    <Button type="button" variant="ghost" size="icon" aria-label="Quitar" onClick={() => quitarFila('equipos_medicos', i)}>
                                        <Trash2 className="size-4 text-destructive" />
                                    </Button>
                                </div>
                            ))}
                            <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={() => setData('equipos_medicos', [...data.equipos_medicos, { id: '', minutos_uso: '' }])}
                            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Button asChild variant="outline">
                            <Link href={CirugiaController.index.url()}>Cancelar</Link>
                        </Button>
                    </div>
                </form>
            </div>
