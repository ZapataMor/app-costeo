...
import type { Completitud, CostosKpi, GlosasRecaudo, UtilizacionSalas } from '@/types/costeo';
...
const paneles = [
    {
        ...
        descripcion: 'Composición del costo: talento humano, sala, equipos e insumos.',
        href: '/costeo/componentes',
        icono: ChartColumnBig,
    },
    {
        titulo: 'Outliers de costo',
        descripcion: 'Cirugías con costos atípicos (z-score e IQR) por procedimiento.',
        ...
        <>
            ...
            <div className="flex flex-col gap-4 p-4">
                        ...
                        <Link key={panel.href} href={panel.href} prefetch>
                            <Card className="h-full transition-colors hover:bg-accent/50">
                                <CardHeader>
                                    ...
                                    <CardTitle className="text-base">{panel.titulo}</CardTitle>
                                    <CardDescription>{panel.descripcion}</CardDescription>
                                </CardHeader>
                            </Card>
                        </Link>
                    ))}
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">Completitud de captura por bloque</CardTitle>
                            <CardDescription>
                                Cirugías realizadas con cada bloque de información registrado
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <ul className="space-y-3">
                                {Object.entries(completitud.chequeos).map(([clave, chequeo]) => (
                                    <li key={clave} className="flex items-center gap-3">
                                        <span className="w-44 shrink-0 text-sm">
                                            {etiquetasChequeos[clave] ?? clave}
                                        </span>
                                        <div className="h-2 flex-1 overflow-hidden rounded bg-muted">
                                            <div
                                                className="h-full rounded bg-primary"
                                                style={{ width: `${(chequeo.porcentaje ?? 0) * 100}%` }}
                                            />
                                        </div>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-base">Costo promedio por procedimiento</CardTitle>
                            <CardDescription>Código CUPS y promedio de las cirugías costeadas</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b text-left text-muted-foreground">
                                        <th className="py-2 font-medium">CUPS</th>
                                ...
                                <tbody>
                                    {costos.por_procedimiento.map((fila) => (
                                        <tr key={fila.procedimiento.id} className="border-b last:border-0">
                                            <td className="py-2 font-mono text-xs">{fila.procedimiento.codigo_cups}</td>
                                            <td className="py-2">{fila.procedimiento.nombre}</td>
